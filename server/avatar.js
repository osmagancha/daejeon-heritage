...
const STAGES = [
  ...
  { name: '초학',   hanja: '初學',   need: 20     ,     tier: 1, desc: '천자문을 뗐다. 붓 잡는 법을 배운다.' },
  { name: '학동',   hanja: '學童',   need: 150    ,    tier: 1, desc: '서당에 다닌다. 소리 내어 읽는 것이 즐겁다.' },
  { name: '서동',   hanja: '書童',   need: 500    ,    tier: 1, desc: '스승의 책시중을 든다. 어깨너머로 더 배운다.' },
  { name: '접장',   hanja: '接長',   need: 1200   ,   tier: 1, desc: '또래 중 맏이가 되어 아우들을 가르친다.' },
  { name: '교생',   hanja: '校生',   need: 2200   ,   tier: 2, desc: '향교에 이름을 올렸다. 이제 학생이다.' },

  { name: '유생',   hanja: '儒生',   need: 3600   ,   tier: 2, desc: '유건을 썼다. 경전을 본격으로 읽는다.' },
  { name: '재생',   hanja: '齋生',   need: 5600   ,   tier: 2, desc: '재(齋)에 머물며 밤늦도록 글을 읽는다.' },
  { name: '상재',   hanja: '上齋',   need: 8100   ,   tier: 2, desc: '재생 중에서도 앞자리에 앉는다.' },
  { name: '초시',   hanja: '初試',   need: 11300  ,   tier: 2, desc: '첫 시험을 통과했다. 이름이 알려지기 시작한다.' },
  { name: '생원',   hanja: '生員',   need: 15200  ,  tier: 3, desc: '경전에 밝다는 증표를 얻었다.' },
  { name: '진사',   hanja: '進士',   need: 19900  ,  tier: 3, desc: '시문(詩文)에 밝다. 도포 자락이 어울린다.' },

  { name: '선비',   hanja: '士',     need: 25500  ,  tier: 3, desc: '갓을 갖춰 쓰고 사방을 유람한다.' },
  { name: '처사',   hanja: '處士',   need: 31500  ,  tier: 3, desc: '벼슬에 나가지 않고 학문에 머문다.' },
  { name: '거사',   hanja: '居士',   need: 39000  ,  tier: 3, desc: '산수를 벗 삼아 지낸다. 글이 깊어진다.' },
  { name: '산림',   hanja: '山林',   need: 47500  ,  tier: 4, desc: '초야에 있어도 조정이 그 이름을 안다.' },
  { name: '유현',   hanja: '儒賢',   need: 56500  ,  tier: 4, desc: '묻고 배우러 오는 이가 끊이지 않는다.' },
  { name: '명유',   hanja: '名儒',   need: 67000  ,  tier: 4, desc: '한 고을을 대표하는 학자가 되었다.' },

  { name: '승문',   hanja: '承文',   need: 79000  ,  tier: 5, desc: '문서를 맡아 조정에 들었다. 사모를 썼다.' },
  { name: '한림',   hanja: '翰林',   need: 92000  ,  tier: 5, desc: '임금 곁에서 글을 짓는다.' },
  { name: '도깨비', hanja: '',       need: 106000 ,  tier: 6, emoji: '👹',
    desc: '사람이길 관두었다. 씨름을 걸어오거든 왼다리를 걸면 이긴다.',
    form: { kind: 'ogre', skin: '#7E4A6B', horns: 2, glow: '#C56BE0', club: true } },

  { name: '야차',   hanja: '夜叉',   need: 121500 ,  tier: 6, emoji: '😈',
    desc: '험한 얼굴로 절 문을 지킨다. 무섭게 생겼지만 속은 여리다.',
    form: { kind: 'ogre', skin: '#6E2B2B', horns: 2, fangs: true, glow: '#E2603A' } },

  { name: '해태',   hanja: '獬豸',   need: 138500 ,  tier: 6, emoji: '🦁',
    desc: '옳고 그름을 가린다. 새치기하는 사람을 조용히 노려본다.',
    form: { kind: 'beast', skin: '#B98A3E', horns: 1, mane: true, glow: '#F0C46A' } },

  { name: '삼족오', hanja: '三足烏', need: 157000 ,  tier: 6, emoji: '🐦',
    desc: '해 속에 산다는 세 발 까마귀. 다리가 하나 더 있어 좀처럼 넘어지지 않는다.',
    form: { kind: 'bird', skin: '#2B2622', wings: true, legs: 3, glow: '#F2A23C' } },

  { name: '구미호', hanja: '九尾狐', need: 176500 ,  tier: 6, emoji: '🦊',
    desc: '꼬리가 아홉이다. 앉을 자리를 고르는 데 시간이 오래 걸린다.',
    form: { kind: 'fox', skin: '#D8A05A', tails: 9, glow: '#F5D97E' } },

  { name: '청룡',   hanja: '靑龍',   need: 198000 ,  tier: 7, emoji: '🐉',
    desc: '천 년 만에 승천했다. 동쪽 하늘이 담당 구역이다.',
    form: { kind: 'dragon', skin: '#2E6E8C', horns: 2, whiskers: true, glow: '#6BD5F5' } },

  { name: '산신령', hanja: '山神靈', need: 221000 , tier: 7, emoji: '🧙',
    desc: '계족산에 산다. 도끼를 빠뜨리면 금도끼를 들고 나타난다.',
    form: { kind: 'spirit', skin: '#E8E2D2', beard: true, halo: 1, float: true, glow: '#C9E8B0' } },

  { name: '염라대왕', hanja: '閻羅大王', need: 245500 , tier: 7, emoji: '📕',
    desc: '저승의 장부를 넘긴다. 당신 이름 옆에 스탬프 개수가 적혀 있다.',
    form: { kind: 'ogre', skin: '#5E1B1B', horns: 2, crown: true, beard: true, halo: 1, glow: '#E2603A' } },

  { name: '돌부처', hanja: '石佛',   need: 272000 , tier: 8, emoji: '🗿',
    desc: '한자리에 하도 오래 서 있어 이끼가 앉았다. 지나던 사람이 절을 하고 갔다.',
    form: { kind: 'stone', skin: '#9AA093', halo: 1, moss: true, glow: '#BFD0B0' } },

  { name: '천지신명', hanja: '天地神明', need: 300000 , tier: 8, emoji: '🌌',
...
const TIERS = [
  ...
  { tier: 4, name: '유림', emoji: '📜', wear: '갓과 흰 수염', realm: '사람',
    form: { kind: 'human', hat: 'gat', beard: true } },
  { tier: 5, name: '관료', emoji: '🏛', wear: '사모와 흉배', realm: '사람',
